from model_utils.utils import self_cross_entropy_loss,_softmax_self,Data_modify_target,calculate_score
import argparse
...
from model_utils.dataload import get_data_roadsegment_Porto
...
def train(args):
    ...
    # % ====================== Load data ======================
    # Read  train data    
    trainlist, testlist = get_data_roadsegment_Porto()
    ...
    global_node = W_D_GAT(7366, 8, 256, args.global_embed_dim, 0.5, 0.2, 2).to("cuda")
    ...
    for epoch in range(args.epochs):
        ...
        for step, data in enumerate(train_loader):
            ...
            single_loss = self_cross_entropy_loss(y_pred,batch_tar_r,batch_mask,'porto')
            # 清除网络先前的梯度值
            optimizer.zero_grad()
            ...
            y_pred = _softmax_self(y_pred, batch_mask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--node_X',default='./Porto-data/node_onehot_porto.npy')
    ...
    parser.add_argument('--transition_probability_map',
                        default='./Porto-data/transition_probability_map_node2node.npy')
    ...
    parser.add_argument('--node_mask', default='./Porto-data/node_mask_node2node.npy')

    # embedding
    parser.add_argument('--node_num', type=int, default=7366)
    parser.add_argument('--num_taxi', type=int, default=448)
