from model_utils.utils import self_cross_entropy_loss,_softmax_self_bj,Data_modify_target,calculate_score
import argparse
...
from model_utils.dataload import get_data_roadsegment_bj
from model.model import LocalAggregator,hgt_rn
...
def train(args):
    ...
    # % ====================== Load data ======================
    # Read  train data    
    trainlist, testlist = get_data_roadsegment_bj(file_path)
    ...
    global_node = W_D_GAT(37486, 8, 256, args.global_embed_dim, 0.5, 0.2, 2).to("cuda")
    ...
    optimizer = optim.Adam(params=list(global_node.parameters()) +
                           ...
                           lr=args.lr,weight_decay=5e-1)
    
    criterion_fun = nn.CrossEntropyLoss(ignore_index=-1).to(device=args.device)
    # criterion_fun = nn.MSELoss().to(device=args.device)
    ...
    for epoch in range(args.epochs): 
        ...
        for step, data in enumerate(train_loader):
            ...
            single_loss = self_cross_entropy_loss(y_pred, batch_tar_r, batch_mask, 'bj')
            # 清除网络先前的梯度值
            optimizer.zero_grad()
            ...
            y_pred = _softmax_self_bj(y_pred,batch_mask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--node_X',default= './Bj-random42/node_onehot_1.npy')
    parser.add_argument('--edge_index', default= file_path +'/edge_index_train-v2.npy')
    parser.add_argument('--edge_weight',default= file_path +'/edge_weight_train_min-max-v2.npy')
    parser.add_argument('--edge_r',default= file_path +'/edge_r_train-v2.npy')
    #  probability transition vector
    parser.add_argument('--transition_probability_map',default= file_path + '/transition_probability_map_node2node_bj_dic_v2.npy')
    ...
    parser.add_argument('--node_mask',default= './Bj-random42/node_mask_node2node_bj_dic.npy')

    # embedding
    parser.add_argument('--node_num', type=int, default=37486)
    parser.add_argument('--num_taxi', type=int, default=10356)
